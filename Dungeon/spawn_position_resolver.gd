...
# Handles safe enemy spawn positioning by checking nearby positions
# and ensuring they do not overlap with solid world objects.
...
# Finds a suitable spawn position that is clear of walls.
# It first checks the preferred position, then searches nearby
# positions in different directions if the original position is blocked.
...
# Checks whether the current position is valid and optionally
# checks whether there is a clear path toward the target.
...
# Checks whether the body's collision shape overlaps
# any solid world object.
...
# Checks whether there is a clear path from the body
# to the target position using a physics raycast.
...
# Updates the enemy's home position when the spawn position
